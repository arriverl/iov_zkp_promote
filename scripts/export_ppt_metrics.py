# -*- coding: utf-8 -*-
"""从 results/*.csv 导出 PPT 可直接引用的指标（JSON + Markdown）。"""
...
def build_metrics() -> dict:
    ...
    return {
        ...
        "ppt_bullets": {
            ...
            "slide11_theft": f"证书窃取攻击成功率 {attacks.get('certificate_theft_impersonation', {}).get('success_rate', 0) * 100:.0f}%",
        },
    }
...
def write_outputs() -> None:
    ...
    lines = [
        "# PPT 指标速查（自动生成）",
        "",
        f"- {metrics['ppt_bullets']['slide13_rsu']}",
        f"- {metrics['ppt_bullets']['slide13_e2e']}",
        f"- {metrics['ppt_bullets']['slide13_comm']}",
        f"- {metrics['ppt_bullets']['slide11_theft']}",
        "",
        "完整 JSON：`docs/PPT_METRICS.json`",
    ]
    (DOCS / "PPT_METRICS.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"Wrote {DOCS / 'PPT_METRICS.json'}")
    print(f"Wrote {DOCS / 'PPT_METRICS.md'}")
